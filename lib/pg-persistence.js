...
module.exports = class PgPersistence {
  ...
  async setContactsToPreference(relationship, dayPref, weekPref, monthPref) {
    const UPDATE_CONTACT_TO_PREFERENCE = "UPDATE contacts " +
                                        ` SET dayReminder = $1, weekReminder = $2, monthReminder = $3` + 
                                        ` WHERE category = $4 AND user_id = $5`;

    let result = await dbQuery(UPDATE_CONTACT_TO_PREFERENCE, dayPref, weekPref, monthPref, relationship, this.username);
    return result.rowCount > 0;
  }
